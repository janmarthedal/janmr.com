...
og:
  description: >-
    A speed comparison between MathJax versions 2.4 and 2.5, following up on earlier
    experiments with MathJax and KaTeX.
...
So switching to version 2.5 seems like a no-brainer. It's faster and there seems to be no difference in the visual appearance. I have no intention of using the [CHTML preview extension](http://docs.mathjax.org/en/v2.5-latest/options/CHTML-preview.html), however. An annoying page reflow is still needed when switching from the preview to the "real" typesetting and I would rather see the final page as soon as possible. (Note that the preview extension is *enabled* by default.) My [use of KaTeX](/posts/mathjax-katex-and-a-lot-of-math/) for most of the inline, simple math, also makes the need for a fast preview even smaller.
