...
- `poisson-1d-rough-source.svg`
  - Three stacked panels sharing the $x$ axis for the problem
    $-u'' = f$ on $(0,1)$, $u(0) = u(1) = 0$, with $f = 1$ on $(0,1/2)$ and $f = 0$ on $(1/2,1)$:
    the discontinuous source, the solution, and the derivative with its kink at $x = 1/2$.
  - Generated by `assets-src/finite-element-method/poisson_1d_rough_source.py`.
...
- `discrete-stiffness-sparsity.svg`
  - The sparsity pattern of the stiffness matrix $K$ for a small triangulated
    mesh (64 arbitrarily numbered nodes, ~98 elements): a grid of filled/empty
    squares.
  - Generated by `assets-src/finite-element-method/discrete_stiffness_sparsity.py`.
...
`CLAUDE.md`: the post must explain the underlying theory independently, and the
...
5. `continuous/poisson-weak-form.md` — **Weak Formulation of the Poisson Problem**
   ...
   - State simple sufficient conditions for well-posedness and note that they hold
     for essentially every problem the finite element method is applied to.

6. `continuous/poisson-weak-example.md` — **Example of Weak Solution to the Poisson Problem**
   - Work through the 1D problem $-u'' = f$ on $(0,1)$ with homogeneous Dirichlet
     conditions and a source term that is discontinuous at the midpoint.
   - Show that the solution is continuously differentiable but has a jump in the
     second derivative, so the strong form has no solution while the weak one does.
   - Illustrate using `poisson-1d-rough-source.svg`.
   - Note that discontinuous data are common in applications, and that the
     piecewise polynomial solution hints at the shape of finite element spaces.

7. `continuous/elliptic-weak-form.md` — **Weak Formulation of Elliptic PDEs**
...
9. `discrete/basis-to-shape-functions.md` — **From Basis to Shape Functions**
   ...
   - Include the illustration `discrete-stiffness-sparsity.svg` to make the
     sparsity argument concrete: the sparsity pattern of $K$ for a small mesh.
...
10. `discrete/continuous-galerkin.md` — **Continuous Galerkin**
...
11. `ref-domains/index.md` — **Reference Domains and Quadrature**
...
12. `ref-domains/line.md` — **The Line Domain**
...
13. `ref-domains/triangle.md` — **The Triangle Domain**
...
14. `ref-domains/quadrilateral.md` — **The Quadrilateral Domain**
...
17. `elements/index.md` — **Shape Functions on Reference Elements**
...
18. `elements/line2.md` — **The Line2 Element**
...
19. `elements/line3.md` — **The Line3 Element**
...
20. `elements/tri3.md` — **The Tri3 Element**
...
21. `elements/tri6.md` — **The Tri6 Element**
...
22. `elements/quad4.md` — **The Quad4 Element**
...
23. `elements/quad9.md` — **The Quad9 Element**
...
24. `elements/tet4.md` — **The Tet4 Element**
...
25. `elements/hex8.md` — **The Hex8 Element**
...
26. `meshes/representation.md` (PLANNED) — **Mesh Representation**
    - Library code: Mesh, ElementGroup

27. `meshes/boundaries.md` (PLANNED) — **Mesh Boundaries**
    - Library code: extract_boundary, tag_boundary_faces

28. `meshes/meshio-interop.md` (PLANNED) — **MeshIO Interoperability**
    - Library code: from_meshio, to_meshio

29. `assembly/computing-jacobian.md` (PLANNED) — **Computing the Jacobian**
    - Library code: compute_physical_gradients

30. `assembly/bilinear-form.md` (PLANNED) — **Bilinear Form Assembly**
    - Library code: assemble_bilinear_form

31. `assembly/load-vector.md` (PLANNED) — **Load Vector Assembly**
