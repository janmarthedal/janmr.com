...
I recently tried using KaTeX for the math on this blog. This was an obvious thing to try since this blog is statically generated using [Jekyll](http://jekyllrb.com). When each instance of LaTeX markup was processed as part of the Markdown to HTML conversion, a small ([node.js](http://nodejs.org)) web service wrapping the KaTeX libray was queried to produce the corresponding HTML code.

The KaTeX conversion failed in about 10% of time, though, because unsupported LaTeX commands/symbols were being used (mostly multi-line environments). In these cases the conversion would fall back on using MathJax. (It should be noted that some of the unsupported symbols were easy to add. These additions along with a few bug fixes can be found in my [KaTeX fork](https://github.com/janmarthedal/KaTeX).)
...
So it took an average of about one second to load a page and typeset the math. That didn't sound too bad. But many of the 45 posts contained no or little math, and some contained *a lot*. So focusing on the 10 posts that contained most math items, the results were:
...
I won't attempt to make any general conclusions regarding the speed of MathJax and KaTeX. But in my case, with this blog, introducing KaTeX together with MathJax turned out to be a good fit.
