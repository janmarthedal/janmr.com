...
magick ../base-with-logo.png \
  ...
  -pointsize 24 -annotate +0-50 "First post in a series of six on multiple-precision arithmetic" \
  \( /tmp/equation.png -resize 40% \) -geometry +0+80 -composite \
  $OUT/multiple-precision-01.png

# Clean up temporary files
rm /tmp/equation.*

echo "Generated: multiple-precision-01.png"
