#!/usr/bin/env python3
"""
Create an SVG plot of June Solstice occurrences by day and hour
"""
...
def main():
    # Read the CSV file and extract June Solstice column
    june_solstices_all = []
    june_solstices_2025_2075 = []

    with open('seasons.csv', 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['June Solstice']:
                june_solstices_all.append(row['June Solstice'])
                # Extract year for filtering
                year = int(row['Year'])
                if 2025 <= year <= 2075:
                    june_solstices_2025_2075.append(row['June Solstice'])

    # Map each timestamp to (day, hour) for all data
    day_hour_pairs_all = []
    for timestamp in june_solstices_all:
    ...
    for timestamp in june_solstices_2025_2075:
        day, hour = parse_timestamp(timestamp)
    ...
    # Create all possible (day, hour) combinations in range (Jun 20 00:00 to Jun 22 23:00)
    all_combinations = []
    # Jun 20: all hours
    ...
    # Customize the plot
    ax.set_xlabel('June Day / Hour (UTC)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Number of Occurrences', fontsize=12, fontweight='bold')
    ax.set_title('June Solstice Distribution by Day and Hour (1900-2100 with 2025-2075 Overlay)',
    ...
    # Save as SVG
    output_file = 'june_solstice_histogram.svg'
    plt.savefig(output_file, format='svg', dpi=300, bbox_inches='tight')
    print(f"Plot saved as {output_file}")

    # Print summary statistics
    print(f"\nAnalyzed {total_all} June solstice timestamps (1900-2100)")
    print(f"Analyzed {total_2025_2075} June solstice timestamps (2025-2075)")
    days = sorted(set(day for day, hour in day_hour_pairs_all))
    print(f"Date range: Jun {min(days)} to Jun {max(days)}")
    ...
    for day in sorted(day_counts_all.keys()):
        print(f"  June {day}: {day_counts_all[day]}")
    print("\nOccurrences by day (2025-2075):")
    for day in sorted(day_counts_2025_2075.keys()):
        print(f"  June {day}: {day_counts_2025_2075[day]}")
