#!/usr/bin/env python3
"""
Create an SVG plot of September Equinox occurrences by day and hour
"""
...
def main():
    # Read the CSV file and extract September Equinox column
    september_equinoxes_all = []
    september_equinoxes_2025_2075 = []

    with open('seasons.csv', 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['September Equinox']:
                september_equinoxes_all.append(row['September Equinox'])
                # Extract year for filtering
                year = int(row['Year'])
                if 2025 <= year <= 2075:
                    september_equinoxes_2025_2075.append(row['September Equinox'])

    # Map each timestamp to (day, hour) for all data
    day_hour_pairs_all = []
    for timestamp in september_equinoxes_all:
    ...
    for timestamp in september_equinoxes_2025_2075:
        day, hour = parse_timestamp(timestamp)
    ...
    # Create all possible (day, hour) combinations in range (Sep 21 22:00 to Sep 24 05:00)
    all_combinations = []
    # Sep 21: hours 22-23
    ...
    # Sep 24: hours 0-5
    for hour in range(6):
        all_combinations.append((24, hour))
    ...
    # Customize the plot
    ax.set_xlabel('September Day / Hour (UTC)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Number of Occurrences', fontsize=12, fontweight='bold')
    ax.set_title('September Equinox Distribution by Day and Hour (1900-2100 with 2025-2075 Overlay)',
    ...
    # Save as SVG
    output_file = 'september_equinox_histogram.svg'
    plt.savefig(output_file, format='svg', dpi=300, bbox_inches='tight')
    print(f"Plot saved as {output_file}")

    # Print summary statistics
    print(f"\nAnalyzed {total_all} September equinox timestamps (1900-2100)")
    print(f"Analyzed {total_2025_2075} September equinox timestamps (2025-2075)")
    days = sorted(set(day for day, hour in day_hour_pairs_all))
    print(f"Date range: Sep {min(days)} to Sep {max(days)}")
    ...
    for day in sorted(day_counts_all.keys()):
        print(f"  September {day}: {day_counts_all[day]}")
    print("\nOccurrences by day (2025-2075):")
    for day in sorted(day_counts_2025_2075.keys()):
        print(f"  September {day}: {day_counts_2025_2075[day]}")
